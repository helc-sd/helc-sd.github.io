...
class ContractController extends IndexController
{
    public function index(){
        ...
        Session::set('bustaff_name',$staff_name);
        ...
        // 按条件查询数据并调用分页
        $bustaffs = $Bustaff->where('staff_name', 'like', '%' . $staff_name . '%')->paginate($pageSize, false, [
            'query'=>[
    ...
    {
        $result= Db::query("
SELECT contract_id,SUBSTRING(branch,1,2) as company,sales_clerk,contract_num,send_back_date,audit_date,audit_version,delivery_date,if_nonstandard,IF(send_date='0000-00-00','',send_date) AS send_date
FROM helc_contract 
WHERE DATE_SUB(CURDATE(), INTERVAL 1 MONTH) <= date(send_back_date) OR DATE_SUB(CURDATE(), INTERVAL 1 MONTH) <= date(delivery_date) 
ORDER BY send_back_date DESC
");
    ...
    function export(){
        ...
        $xlsData  = Db::name('bustaff')->where('staff_name','like','%'.$staff_name.'%')->select();
